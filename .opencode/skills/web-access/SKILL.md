...
> **本项目专属配置（隐私硬性要求）**：
> 1. 本 skill 只允许连接专用隔离浏览器实例 `web-access-profile/`（唯一可发现的浏览器，日常 Chrome/Edge 已被移除出发现列表，agent 物理上无法连接它们）。
> 2. 运行环境为 Windows PowerShell：命令中所有 `curl` 一律用 `curl.exe`（PowerShell 里 `curl` 是 `Invoke-WebRequest` 别名，参数不兼容）。
> 3. skill 目录（下文用 `${CLAUDE_SKILL_DIR}` 表示的）固定为工作区相对路径 `.opencode/skills/web-access`。
> 4. 每次连接前确认专用实例正在运行（见 `scripts/start-web-access-profile.ps1`），失败则明确告知用户先启动它。
> 5. 本环境无 `pkill`：需要重启 cdp-proxy 时用 `Stop-Process -Name node -Force` 后重跑 check-deps.mjs（仅在此 skill 专用 Node 进程无其它 node 任务时使用）。
...
按脚本输出处理：
- `exit 0` → 继续
- `exit 2` → 需询问用户偏好，写入 `.opencode/skills/web-access/config.env` 的 `WEB_ACCESS_BROWSER`
...
```bash
node ".opencode/skills/web-access/scripts/find-url.mjs" [关键词...] [--only bookmarks|history] [--browser chrome|edge] [--limit N] [--since 1d|7h|YYYY-MM-DD] [--sort recent|visits]
```
