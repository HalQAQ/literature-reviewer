...
// 加新浏览器：只改这里
//
// 【隐私硬性要求】本实例只允许连接专用隔离浏览器（web-access-profile/）。
// 绝不发现/连接用户的日常 Chrome/Edge，避免 agent 读取任何个人数据。
export function knownBrowsers() {
  const profileDir = path.resolve(SKILL_ROOT, '..', '..', '..', 'web-access-profile');
  return [
    { id: 'dedicated', label: 'Dedicated HKU Profile', devToolsPath: path.join(profileDir, 'DevToolsActivePort'), fixedPort: 9222 },
  ];
}
...
async function detectAll() {
  ...
  for (const browser of knownBrowsers()) {
    ...
    let port = 0;
    let wsPath = null;
    try { content = fs.readFileSync(browser.devToolsPath, 'utf8'); } catch { content = null; }
    if (content) {
      const lines = content.trim().split(/\r?\n/).filter(Boolean);
      port = parseInt(lines[0], 10);
      wsPath = lines[1] || null;
    }
    // 专用实例固定调试端口兜底：DevToolsActivePort 可能未写入，但端口已监听
    if (!(port > 0 && port < 65536) && browser.fixedPort && (await checkPort(browser.fixedPort))) {
      port = browser.fixedPort;
    }
