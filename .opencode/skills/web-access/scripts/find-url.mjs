...
// 加新浏览器：只改这里
//
// 【隐私硬性要求】只允许检索专用隔离浏览器（web-access-profile/）的书签/历史，
// 绝不读取用户日常 Chrome/Edge 的任何数据。
function knownBrowserDataDirs() {
  const profileDir = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', '..', '..', '..', 'web-access-profile');
  return [
    { id: 'dedicated', label: 'Dedicated HKU Profile', dir: profileDir },
  ];
}
